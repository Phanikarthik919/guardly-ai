  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "Payment required, please add funds to your Lovable workspace." }), {
          status: 402,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    // Lovable AI already returns OpenAI-compatible SSE format, pass through directly
